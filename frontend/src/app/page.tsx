    ...
    <div className="relative overflow-hidden">
      ...
      <section className="max-w-7xl mx-auto px-6 pt-24 pb-20 text-center relative z-10">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-sky-500/30 bg-sky-500/5 text-sky-400 text-xs font-semibold mb-8 tracking-wide uppercase">
          ...
          Paystack Subaccount Infrastructure
        </div>
        ...
        <p className="text-lg md:text-xl text-slate-400 max-w-2xl mx-auto mb-10 leading-relaxed font-light">
          Onboard merchants onto Paystack subaccounts in minutes. Every transaction automatically splits your platform fee — zero manual settlement work.
        </p>
        ...
        {/* Social proof */}
        <p className="text-xs text-slate-600 mt-6">No card required · Test mode ready · Paystack-backed settlement</p>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-6 pb-16">
        <div className="glass gradient-border rounded-2xl overflow-hidden">
          ...
          <div className="flex items-center gap-1.5 px-5 py-3 border-b border-slate-800/60">
            ...
            <span className="ml-3 text-xs text-slate-500 font-mono">Charge API Example</span>
          </div>
          <pre className="p-6 text-sm font-mono leading-relaxed overflow-x-auto" style={{ color: '#94a3b8' }}>
            <span style={{ color: '#64748b' }}># 1. Onboard a merchant (one-time)</span>{'\n'}
            <span style={{ color: '#38bdf8' }}>curl</span>{' '}
            <span style={{ color: '#f59e0b' }}>-X POST</span>{' '}
            <span style={{ color: '#34d399' }}>https://api.skrillpay.com/v1/merchants/onboard</span>{' '}
            <span style={{ color: '#94a3b8' }}>-d</span>{' '}
            <span style={{ color: '#f1f5f9' }}>{'\'{"email":"merchant@store.com","business_name":"TechMart","bank_account_number":"0123456789","bank_code":"057"}\''}</span>{'\n\n'}
            <span style={{ color: '#64748b' }}># 2. Initialize a charge with automatic platform fee split</span>{'\n'}
            <span style={{ color: '#38bdf8' }}>curl</span>{' '}
            <span style={{ color: '#f59e0b' }}>-X POST</span>{' '}
            ...
            <span style={{ color: '#64748b' }}># Returns: authorization_url for Paystack checkout</span>
          </pre>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 pb-24 grid md:grid-cols-3 gap-6">
        {[
          {
            icon: '🏦',
            title: 'Instant Subaccounts',
            desc: 'KYC form triggers Paystack subaccount creation in real-time. Merchants are onboarded and settlement-ready in minutes.',
            color: 'sky',
          },
          {
            icon: '✂️',
            title: 'Automatic Fee Splits',
            desc: 'Define percentage or flat fees per merchant. Paystack splits the funds on every transaction — nothing to reconcile.',
            color: 'indigo',
          },
          {
            icon: '🛡️',
          ...
          },
          {
            icon: '🇳🇬',
            title: 'Built for NGN',
            desc: 'All monetary amounts stored in Kobo (integer subunits) to eliminate floating-point rounding errors. Paystack-native.',
            color: 'emerald',
          },
        ].map((f) => (
          <div
          ...
          >
            <div
              className={`w-11 h-11 rounded-xl flex items-center justify-center text-xl mb-5 transition-transform group-hover:scale-110 ${
                f.color === 'sky' ? 'bg-sky-500/10' : f.color === 'indigo' ? 'bg-indigo-500/10' : 'bg-emerald-500/10'
              }`}
            >
              {f.icon}
            </div>
      ...
      <section className="max-w-7xl mx-auto px-6 pb-24">
        <div className="glass gradient-border rounded-3xl p-12 text-center" style={{ background: 'linear-gradient(135deg, rgba(56,189,248,0.05) 0%, rgba(129,140,248,0.05) 100%)' }}>
          ...
          <p className="text-slate-400 mb-8 max-w-lg mx-auto">
            Onboard your first merchant and start accepting payments in minutes. Switch to live mode with your Paystack production key.
          </p>
          <Link
            href="/auth"
